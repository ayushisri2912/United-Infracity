...
import { X, User, Lock, Mail, Phone, Shield, CheckCircle2 } from 'lucide-react';

export default function UserModal({ isOpen, onClose, onOpenEnquire }) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-[#FDFBF7] max-w-md w-full rounded-3xl overflow-hidden shadow-2xl border border-[#D4AF37]/40 relative">
        ...
        <div className="bg-stone-900 text-white p-6 relative">
          <button
            onClick={onClose}
            className="absolute top-5 right-5 p-1.5 rounded-full bg-stone-800 text-stone-400 hover:text-white"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        <div className="p-6">
          {isLoggedIn ? (
            <div className="text-center space-y-4 py-4">
              ...
              <div className="space-y-2 pt-2">
                <button
                  ...
                  className="w-full py-3 bg-[#D4AF37] text-white font-bold text-xs uppercase tracking-wider rounded-xl shadow-md"
                >
                  Book Private Site Visit
                </button>
                <button
                  onClick={() => setIsLoggedIn(false)}
                  className="w-full py-2.5 bg-stone-100 hover:bg-stone-200 text-stone-700 font-semibold text-xs rounded-xl"
                >
                  Sign Out
                </button>
              </div>
            ...
            <form onSubmit={handleLoginSubmit} className="space-y-4">
              ...
              <button
                type="submit"
                className="w-full py-3.5 bg-[#1A1A1A] hover:bg-[#D4AF37] text-white hover:text-[#1A1A1A] font-bold text-xs uppercase tracking-widest rounded-xl transition-all shadow-md"
              >
                Access Portal
              </button>
            </form>
